...
def start_message(message):
    bot.send_message(message.chat.id, 'Я очень вежливый бот: могу поздороваться или попрощаться в ответ, твоя любовь ко мне всегда будет взаимна. \n /start - начать общение \n /dice - кинуть кость \n Или напиши "Гороскоп", чтобы узнать предсказание на сегодня')
...
def send_text(message):
    ...
    elif message.text.lower() == 'гороскоп':#Гороскоп
        # Пишем приветствие
...
#             l[i][j] = d[i][j+1]
#     for i in l:
#         answer.append(i[3]) #answer - ответы
#         shuffle(i)
#
#     keyboard.add(*l[0])
#     return keyboard
...
#     keyboard = ff(keyboard)
#     bot.send_message(message.chat.id, d[0][0], reply_markup=keyboard)
#
# count = 0
# # Вот простой обработчик, когда пользователь нажимает кнопку с текстом
# @bot.message_handler(content_types=["text"])
# def func1(message):
#     global count
